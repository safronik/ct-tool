...
class ScannerQueue
{
            ...
            if(
                ...
                if(empty($result['error'])){
                    ...
                    if(empty($result['error'])){
                        ...
                        $sql = 'INSERT INTO ' . SPBC_TBL_SCAN_FILES . ' (`fast_hash`, `path`, `real_full_hash`, `source_type`, `source`, `version`, `checked`, `status`) VALUES ';
                        $data = array();
                        foreach($result['checksums'] as $path => $real_full_hash){
                            $path = $is_windows ? str_replace('/', '\\', $path) : $path;
                            $fast_hash = md5($path);
                            $path = addslashes($path);
                            $data[] = sprintf('("%s","%s","%s","CORE", "wordpress", "%s", "YES", "OK")', $fast_hash, $path, $real_full_hash, $wp_version);
                        }
        ...
        foreach( array('plugins', 'themes') as $modules_type ){
            ...
            foreach($modules as $module_slug => $module){
                ...
                if( empty($result_hashes['error']) ){
                    ...
                    $sql_hat = 'INSERT INTO ' . SPBC_TBL_SCAN_FILES . ' (`fast_hash`, `path`, `real_full_hash`, `source_type`, `source`, `source_status`, `version`, `checked`, `status`) VALUES ';
                    $sql_values = array();
                    foreach($result_hashes as $value){
                        ...
                        $sql_values[] = "('$fast_hash', '$path', '$real_full_hash', '" . strtoupper($modules_type_singular) . "', '$module_slug', '$source_status', '{$module['Version']}', 'YES', 'OK')";
                    }
                    if( $sql_values ){
     ...
     * @return array|int
     */
    public function countFilesByStatusAndChecked( $status = null, $checked = null )
    {
        $status  = stripslashes( Request::get('status') )  ?: $status;
        $checked = stripslashes( Request::get('checked') ) ?: $checked;
    
        if( ! preg_match( '#^[A-Z,_]+$#', $status ) || ! preg_match( '#^[A-Z,_]+$#', $checked ) ){
            return array('error' => 'BAD_PARAMS');
        }
        
        $status  = is_string($status) ? explode(',', $status) : $status;
        $status  = '"' . implode('","', $status) . '"';
        $checked = is_string($checked) ? explode(',', $checked) : $checked;
        $checked = '"' . implode('","', $checked) . '"';
        
        $result = $this->db->fetch(
            'SELECT COUNT(fast_hash) AS cnt'
            .' FROM '.SPBC_TBL_SCAN_FILES
            .' WHERE checked IN ('.$checked.') AND status IN ('.$status.');' // No need to validate or sanitize, already did
        );
    ...
    public function countFileSystem($path_to_scan = ABSPATH){
        ...
        $init_params  = array(
            ...
            'dir_exceptions'  => array(SPBC_PLUGIN_DIR . 'quarantine', 'node_modules', 'vendor')
        );
    ...
    public function signature_analysis($status = 'UNKNOWN,MODIFIED,OK,INFECTED', $offset = null, $amount = null){
        ...
        if( $offset === 0 ){
            $total = $this->countFilesByStatusAndChecked( $status_raw, 'NO,YES_HEURISTIC' );
            $total= $total['total'];
        }
        
        // Get files to check for this iteration
        $files = $this->db->fetch_all(
            'SELECT path, source_type, source, version, status, checked, fast_hash, real_full_hash, full_hash, weak_spots, difference, severity, size'
            .' FROM ' . SPBC_TBL_SCAN_FILES
    ...
    public function heuristic_analysis($status = 'UNKNOWN,MODIFIED,OK,INFECTED', $offset = null, $amount = null){
        ...
        // Count total files if offset is 0
        if( $offset === 0 ){
            $total = $this->countFilesByStatusAndChecked( $status_raw, 'NO,YES_SIGNATURE' );
            $total= $total['total'];
        }
        ...
        $files = $this->db->fetch_all(
            'SELECT path, source_type, source, version, status, checked, fast_hash, real_full_hash, full_hash, weak_spots, difference, severity, size'
            .' FROM ' . SPBC_TBL_SCAN_FILES
            ." WHERE checked IN ('NO', 'YES_SIGNATURE') AND status IN ($status)"
            ." LIMIT 1000"
        );
